...
const App: React.FC = () => {
  return (
    <div className="p-4 flex items-center justify-center flex-col">
      <div className="bg-gradient-to-tr from-yellow-400 to-purple-600 p-1 rounded-full mt-5">
        <img
        ...
        {contactList.map((contact, index) => {
          return (
            <div onClick={() => {
              ...
              <img src={`/assets/${contact.icon}.png`} className="w-[30px] h-[30px]" />
            <div className="flex items-center justify-between w-full">
            <div>{contact.platform}</div>
            {contact.disabled && <div className="text-black font-thin text-[12px] ">Em breve</div>}
            </div>
          </div>
          )
        })} 
      ...
      <div className="text-center mt-5 text-[12px]">
        ...
        <span className="font-thin text-[12px]">Este projeto esta open-source no meu GitHub.</span>
    </div>
  );
};
